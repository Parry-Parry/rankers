...
class Ranker(PreTrainedModel):
    ...
    def to_pyterrier(self, batch_size=None):
        assert self.transformer_class is not None, "transformer_class must be set by subclasses, do you have pyterrier installed?"
        return self.transformer_class.from_model(self.model, self.tokenizer, text_field='text', batch_size=batch_size)
