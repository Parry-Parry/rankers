...
def MarginMSELoss(pred : Tensor, labels : Tensor, **kwargs):
    return F.mse_loss(residual(pred), residual(labels), reduction='mean')

def HingeLoss(pred : Tensor, labels : Tensor, margin : int = 1, **kwargs):
...
def ClearLoss(pred : Tensor, labels : Tensor, margin : int = 1, **kwargs):
    margin_b = margin - residual(labels)
    return torch.mean(F.relu(margin_b - residual(pred)))

def ContrastiveLoss(pred : Tensor, **kwargs):
