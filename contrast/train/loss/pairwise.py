...
def MarginMSELoss(pred : Tensor, labels : Tensor, reduction='none', **kwargs):
    return F.mse_loss(residual(pred), residual(labels), reduction=reduction)

def HingeLoss(pred : Tensor, labels : Tensor, margin : int = 1, reduction='none', **kwargs):
...
def ClearLoss(pred : Tensor, labels : Tensor, margin : int = 1, reduction='none', **kwargs):
    margin_b = margin - residual(labels)
    return reduce(F.relu(margin_b - residual(pred)), reduction)

def ContrastiveLoss(pred : Tensor, reduction='none', **kwargs):
